...
summary: Para aumentar a capacidade de storage, você pode adicionar um ou dois compartimentos de expansão a um SG6260 implantado em um sistema StorageGRID. 
---
= Adicionar compartimento de expansão ao SG6260 implantado
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Para aumentar a capacidade de storage, você pode adicionar um ou dois compartimentos de expansão a um SG6260 já implantado em um sistema StorageGRID.

.Sobre esta tarefa
Para adicionar um compartimento de expansão, execute estas etapas de alto nível:

* Instale o hardware no gabinete ou rack.
* Coloque o SG6260 em modo de manutenção.
...
A conclusão do procedimento para um ou dois compartimentos de expansão deve levar uma hora ou menos por nó do dispositivo. Para minimizar o tempo de inatividade, as etapas a seguir orientam você a instalar os novos compartimentos de expansão e as unidades antes de colocar o SG6260 em modo de manutenção. As etapas restantes devem levar aproximadamente de 20 a 30 minutos por nó do dispositivo.

.Antes de começar
* Você deve ter a senha de provisionamento.
* Você precisa estar executando StorageGRID 12.0 ou posterior.
* Você tem o compartimento de expansão e dois cabos SAS para cada compartimento de expansão.
* Você já sabe link:locating-sg6200-in-data-center.html["fisicamente localizado o controlador"] onde está adicionando o compartimento de expansão no data center.


.Passos
. Siga as instruções para link:../installconfig/sg6260-installing-60-drive-shelves-into-cabinet-or-rack.html["instalar gavetas de 60 unidades em um gabinete ou rack"].
. Do Gerenciador de Grade, link:../commonhardware/placing-appliance-into-maintenance-mode.html["Coloque o controlador SG6200-CN em modo de manutenção"].
...
+
image::../media/expansion_shelves_connections_sg6260.png[Conexões do compartimento de expansão SG6260]
...
+
Exemplos de mensagens que podem aparecer na página inicial, dependendo do número de compartimentos de expansão existentes ou novos, conforme a seguir:
...
+
Quando a reinicialização estiver concluída, a guia *Tarefas* será exibida com opções para reinicializar o nó ou colocar o dispositivo de storage em modo de manutenção.

. Verifique o status do nó de storage do dispositivo e dos novos compartimentos de expansão.
+
.. No Grid Manager, selecione *Nodes* e verifique se o Storage Node do appliance possui um ícone de marca de seleção verde.
+
