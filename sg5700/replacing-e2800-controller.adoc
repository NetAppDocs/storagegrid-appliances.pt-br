...
CAUTION: *Possível dano ao hardware* -- A aba metálica na parte frontal da bateria deve estar completamente inserida no slot do controlador (como mostrado na primeira figura). Se a bateria não for instalada corretamente (como mostrado na segunda figura), a aba metálica poderá entrar em contato com a placa controladora, causando danos.
...
** *Incorreto -- A aba metálica da bateria não está inserida no slot do controlador:*
+
