...
---
= Substituir um disco rígido em um SG100 ou SG1000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Você também pode usar o Gerenciador de Grade para monitorar o status dos SSDs. Selecione *Nós*. Em seguida, selecione  `*Appliance Node*` > *Hardware*. Se uma unidade falhar, o campo Modo RAID de Armazenamento exibirá uma mensagem informando qual unidade apresentou falha.
...
+
A unidade é reconstruída automaticamente com dados espelhados da unidade em funcionamento. Você pode verificar o status da reconstrução usando o Grid Manager. Selecione *Nodes*. Em seguida, selecione  `*Appliance Node*` > *Hardware*. O campo Storage RAID Mode exibirá a mensagem "`rebuilding`" até que a unidade seja completamente reconstruída.
