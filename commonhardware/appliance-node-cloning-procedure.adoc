...
summary: 'O processo de clonagem pode levar vários dias para transferir dados entre o nó de origem (o dispositivo está sendo substituído) e o dispositivo de destino (novo).' 
---
= Clonar um nó de dispositivo StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Se existirem problemas que impeçam a clonagem, *Iniciar Clonagem* não estará habilitado e os problemas que você precisa resolver serão listados como o *Estado da conexão*. Esses problemas são listados na página inicial do StorageGRID Appliance Installer, tanto no nó de origem quanto no appliance de destino. Apenas um problema é exibido por vez e o estado é atualizado automaticamente conforme as condições mudam. Resolva todos os problemas de clonagem para habilitar *Iniciar Clonagem*.
...
NOTE: Se você encontrar um conflito de endereço IP ao final do processo de clonagem, verifique se não havia concessões DHCP permanentes atribuídas a nenhum dos endereços MAC das portas que tinham endereços IP estáticos no nó de origem e que estavam habilitadas para DHCP no nó de destino. Remova ou atualize as concessões DHCP permanentes no servidor DHCP para resolver esse conflito.
