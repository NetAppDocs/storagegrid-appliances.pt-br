...
---
= Verifique o componente a ser substituído no SG120 ou SG1200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Antes de começar
* Você possui o número de série do dispositivo onde o componente precisa ser substituído.
...
.Sobre esta tarefa
Utilize este procedimento para identificar o aparelho com defeito de hardware e qual dos componentes substituíveis não está funcionando corretamente. Componentes que podem ser identificados para substituição incluem:
...
.. Na tabela da página Nós, selecione o nome do dispositivo StorageGRID com o componente que apresentou falha.
...
+
*** Se a seção StorageGRID Appliance no Gerenciador de Grid não for exibida, o nó selecionado não é um dispositivo StorageGRID. Selecione um nó diferente na visualização em árvore.
...
link:turning-sg120-and-sg1200-identify-led-on-and-off.html["Ligue o LED de identificação do aparelho"]
