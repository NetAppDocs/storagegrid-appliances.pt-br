...
---
= Automatize a instalação e configuração do dispositivo StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
** Política de hash de transmissão LACP e taxa de PDU
...
* Gere um arquivo JSON contendo os detalhes de configuração. Trabalhe com seu consultor de Serviços Profissionais da NetApp para usar o link:https://configbuilder.netapp.com/Index.aspx["Ferramenta NetApp ConfigBuilder"^] (login necessário) para concluir estas etapas:
+
...
. Na seção Carregar JSON, procure e selecione o arquivo de configuração JSON que você deseja carregar.
+
