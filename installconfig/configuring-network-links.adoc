...
---
= Configure os links de rede para um dispositivo de storage StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Sobre esta tarefa
Você só precisa configurar as definições na página Configuração de Link se quiser usar valores diferentes de<<default_values,configurações padrão>> .


NOTE: As alterações na taxa de PDU do LACP feitas seguindo essas instruções permanecem persistentes no ambiente StorageGRID .  Para fazer alterações temporárias na taxa de PDU LACP ao executar operações de manutenção em componentes de rede instalados em seu dispositivo, consulte https://docs.netapp.com/us-en/storagegrid/maintain/changing-nodes-network-configuration.html#temporarily-changing-the-lacp-pdu-rate["Alteração temporária da taxa de PDU do LACP"^] .

[[port_config_tables]]As figuras e tabelas resumem as opções para o modo de vínculo de porta e o modo de vínculo de rede para cada dispositivo.  Veja o seguinte para mais informações:
...
* link:gathering-installation-information-sg120-and-sg1200.html#port-bond-modes["Modos de ligação de portas (SG1200 e SG120)"]
...
* link:gathering-installation-information-sg6200.html#port-bond-modes["Modos de ligação de portas (SGF6212 e SG6200-CN)"]
...
[cols="1a,2a,2a"]
|===
| Modo de ligação de rede | Rede do cliente desabilitada | Rede do cliente habilitada (padrão) 
...
* As portas 1 e 3 não são usadas.
* Uma etiqueta VLAN é opcional.
* A taxa de PDU do LACP e os valores da política de hash de transmissão do LACP podem ser especificados na seção Rede de grade.

 a| 
* As portas 2 e 4 usam uma ligação LACP para a rede de Grade.
* As portas 1 e 3 usam uma ligação LACP para a rede de clientes.
* Tags VLAN podem ser especificadas para ambas as redes para a conveniência do administrador de rede.
* Os valores da taxa de PDU do LACP e da política de hash de transmissão do LACP podem ser especificados nas seções Rede de grade e Rede de cliente.
...
* Os valores da taxa de PDU do LACP e da política de hash de transmissão do LACP podem ser especificados na seção Configurações do link.
...
+
--
As figuras mostram como as quatro portas de rede do SG120 e do SG1200 são agregadas no modo de agregação de portas fixas (configuração padrão).

SG120:

image::../media/sg120_fixed_port.png[Modo de ligação de porta fixa SG120]

SG1200:

image::../media/sg1200_fixed_port.png[Modo de ligação de porta fixa SG1200]
...
image::../media/sg120_aggregate_ports.png[Modo de ligação de porta agregada SG120]

SG1200:

image::../media/sg1200_aggregate_ports.png[SG1200 Aggregate Port Bond Mode]
...
image::../media/sg120_bonded_management_ports.png[Portas de rede administrativas agregadas SG120]

SG1200:

image::../media/sg1200_bonded_management_ports.png[Portas de rede administrativas agregadas SG1200]
...
image::../media/sgf6212_fixed_port.png[Modo de ligação de porta fixa SGF6212]

*SG6200-CN*:

image::../media/sg6200_cn_fixed_port.png[Modo de ligação de porta fixa SG6200-CN]
...
image::../media/sgf6212_aggregate_ports.png[Modo de ligação de porta agregada SGF6212]

*SG6200*:

image::../media/sg6200_cn_aggregate_ports.png[Modo de ligação de porta agregada SG6200-CN]
...
image::../media/sgf6212_bonded_management_ports.png[Portas de rede administrativas agregadas SGF6212]

*SG6200*:

image::../media/sg6200_cn_bonded_management_ports.png[Portas de rede administrativas agregadas SG6200-CN]
...
NOTE: Para o SG5800, o estado do link para a porta 1 não está disponível no software e deve ser verificado fisicamente usando o LED de status no controlador SG5800.

+
[[default_values]]Na primeira vez que você acessar esta página, os valores padrão serão:

+
** *Link Speed* está definido para *Auto*.
** *Port bond mode* está definido como *Fixed*.
** *A política de hash de transmissão do LACP* está definida como *Camada 2+3*.
** *A taxa de PDU LACP* está definida como *Rápida*.
...
** A *Rede do Cliente* está habilitada.
...
+
Se você planeja usar a velocidade de link de 25 GbE para as portas de rede SG6200, SG6100, SG6000, SG5800 ou SG5700:

+
** Use transcetores SFP28 e cabos Twinax SFP28 ou cabos óticos.
** Para o SG5700, selecione *25GbE* na lista suspensa *Link Speed*.
** Para os modelos SG5800, SG6000, SG6100 ou SG6200, selecione *Auto* na lista suspensa *Link speed*.
...
. Consulte o<<port_config_tables,tabela de configuração de modo de ligação de porta fixa e agregada>> para cada tipo de dispositivo e configure o modo de vinculação de porta e o modo de vinculação de rede para corresponder à sua configuração de rede.
+
Você deve especificar tags VLAN exclusivas para a Grid e as Redes Cliente.  Você pode selecionar valores entre 0 e 4095.
