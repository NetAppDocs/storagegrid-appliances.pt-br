...
---
= Configurar os endereços IP dos dispositivos StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Use o instalador do dispositivo StorageGRID para configurar endereços IP e roteamento para o dispositivo de serviços ou nó de armazenamento nas redes de grade, administração e cliente.
...
.Sobre esta tarefa
Você deve atribuir um endereço IP estático para o dispositivo em cada rede de rede ou de administração conectada ou atribuir um contrato de locação permanente para o endereço no servidor DHCP.  O endereço IP estático ou a configuração DHCP são opcionais para uma rede cliente conectada.

Para habilitar ou desabilitar um link ou alterar a configuração do link, consulte as seguintes instruções:
...
* link:../sg120-1200/changing-link-configuration-of-sg120-and-sg1200-appliance.html["Alterar a configuração de link do appliance de serviços SG120 ou SG1200"]
...
* link:../sg6200/changing-link-configuration-of-sg6200-appliance.html["Alterar a configuração de link do appliance SG6200"]
...
. Para configurar a Rede de Grade, selecione *Estático* ou *DHCP* na seção *Rede de Grade* da página e insira suas configurações de rede.
+
[role="tabbed-block"]
====
.Estático
...
NOTE: Se a Rede do Cliente não estiver habilitada, a rota padrão usará o gateway da Rede Grid.
...
--
====
. Para configurar a Rede de Administração, selecione *Estática* ou *DHCP* na seção *Rede de Administração* da página e insira suas configurações de rede.
+
...
+
Verlink:../installconfig/cabling-appliance.html["Aparelho de cabo"] para o local da Porta de Gerenciamento 1 no seu dispositivo.
...
--
====
. Para configurar a Rede do Cliente, selecione *Estático*, *DHCP* ou *Nenhum* na seção *Rede do Cliente* da página e insira suas configurações de rede.
+

NOTE: Para configurar a Rede do Cliente, certifique-se de que ela esteja habilitada na página Configuração de Link.
...
.Nenhum
--
Selecione *Nenhum* para habilitar a Rede do Cliente sem especificar um endereço IP.  A Rede do Cliente precisa apenas de um endereço IP para acesso direto.  Habilitar a rede do cliente sem um endereço IP permite que você configure as interfaces de VLAN da rede do cliente no StorageGRID.
